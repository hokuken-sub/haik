...
function pkwk_login($pass = '')
{
	...
	{
		return check_passwd($pass, $adminpass);
	}
	else
	{
...
function pkwk_hash_compute($phrase = '', $scheme = '{PHPASS}', $prefix = TRUE, $canonical = FALSE)
{
	...
	if (edit_auth($page, $auth_flag, $exit_flag) && is_editable($page)) {
		...
		if ($exit_flag === FALSE) {
			...
			$body = $title = str_replace('$1',
				h(strip_bracket($page)), '$1 は編集できません。');
			if (is_freeze($page))
				$body .= '(<a href="' . $script . '?cmd=unfreeze&amp;page=' .
					rawurlencode($page) . '">' . '凍結解除' . '</a>)';
			$body = '<p>' . $body . '</p>';
			$msg = str_replace('$1', make_search($page), '$1 は編集できません');
			catbody($title, $msg, $body);
			exit;
		}
	}
}
		...
		if ($contents)
		{
			...
			if ($page_meta->get('close', 'public') !== 'public')
			{
				if ($exit_flag)
				{
					// With exit
					$body = $title = str_replace('$1',
						h(strip_bracket($page)), __('$1 は閲覧できません'));
					$body = '<p>' . $body . '</p>';
					$msg = str_replace('$1', make_search($page), __('$1 は閲覧できません'));
					catbody($title, $msg, $body);
					exit;
...
function edit_auth($page, $auth_flag = TRUE, $exit_flag = TRUE)
{
	...
	return $edit_auth ?  basic_auth($page, $auth_flag, $exit_flag,
		$edit_auth_pages, '$1 は編集できません') : TRUE;
}
...
function basic_auth($page, $auth_flag, $exit_flag, $auth_pages, $title_cannot)
{
	...
	{
		$body = $title = str_replace('$1',
			h(strip_bracket($page)), $title_cannot);
...
function is_login()
{
	...
	if ( ! isset($_SESSION) OR ! isset($_SESSION['usr'])) 
		return FALSE;
	return array_key_exists($_SESSION['usr'],$auth_users);
}

/* End of file auth.php */
/* Location: /app/haik-contents/lib/auth.php */