...
{
	if (! preg_match('/^\|(.+)\|([hHfFcC]|[a-zA-Z0-9_ -]+)?$/', $text, $out)) {
		return Factory_Inline($text);
	} else {
		$ret = new Table($out);
		return $ret;
...
class Heading extends Element
{
	...
	function Heading($root, $text)
	{
		...
		list($text, $this->msg_top, $this->id, $this->autoid) = $root->getAnchor($text, $this->level);
		$this->insert(Factory_Inline($text));
		$this->level++; // h2,h3,h4
	}
	...
	function wrap($string, $tag, $param = '', $canomit = TRUE)
	{
		$secedit = plugin_secedit_wrap($string, $tag, $param, $this->autoid);
	...
	function toString()
	{
		return $this->msg_top .  $this->wrap(parent::toString(),
			'h' . $this->level, ' id="' . $this->id . '"' . ' data-sec-id="'. $this->autoid .'"');
	}
}
...
class HRule extends Element
{
	...
	function HRule($root, $text, $classname)
	{
	...
	function toString()
	{
		if ($this->classname)
		{
			return '<hr class="'.h($this->classname).'">';
		}
		else
		{
			global $hr;
	...
	function TableCell($text, $is_template = FALSE)
	{
		...
		while (preg_match('/^(?:(LEFT|CENTER|RIGHT)|(BG)?COLOR\(([#\w]+)\)|SIZE\((\d+)\)):(.*)$/',
			...
			if ($matches[1]) {
				...
				$this->style[$name] = $name . ':' . h($matches[3]) . ';';
				$text = $matches[5];
			} else if ($matches[4]) {
				$this->style['size'] = 'font-size:' . h($matches[4]) . 'px;';
				$text = $matches[5];
			}
		}
		...
		if( preg_match('/^\|STYLE:(.+)\|$/', $out[0], $ms) ){
			...
			if( isset($style['align']) ) {
				if( isset($style['float']) ){
					...
					$addstyle .= ($style['align']=='left') ?
						'margin-left:0px;margin-right:auto;' : 
						'margin-left:auto;margin-right:10px;';
				}
			}
			
			$this->css_class = 'table';			
			...
			if( isset($style['sortable']) ){
				...
				$qt = get_qt();
				$qt->appendv_once('lib_convert_html_table1', 'beforescript', '<link rel="stylesheet" media="screen" href="'.JS_DIR.'jquery.tablesorter.min.css" type="text/css" />');
				$qt->appendv_once('lib_convert_html_table2', 'beforescript', '<script type="text/javascript" src="'.JS_DIR.'jquery.tablesorter.min.js"></script>
	...
	function toString()
	{
		...
		foreach ($parts as $type => $part)
		{
			...
			foreach (array_keys($this->elements) as $nrow) {
				...
				$part_string .= $this->wrap($row_string, 'tr', $row_class);
			}
			$string .= $this->wrap($part_string, $part);
		}
		
		$class = $this->css_class == '' ? 'table' : $this->css_class;
		$string = $this->wrap($string, 'table', ' class="'.$class.'" '.$this->css_style);
...
class YTable extends Element
{
	...
	function toString()
	{
		...
		foreach ($this->elements as $str)
			$rows .= "\n" . '<tr>' . $str . '</tr>' . "\n";
		$rows = $this->wrap($rows, 'table', ' class="table"');
		return $rows;
	}
}
...
class Align extends Element
{
	...
	{
		$str = parent::toString();
		return $this->wrap($str, 'div', ' style="text-align:' . $this->align . '"');
	}
}
...
class Body extends Element
{
	...
	function parse($lines)
	{
		...
		while (! empty($lines)) {
			...
			// Horizontal Rule
			if (preg_match('/^-{4,}(.*?)(?:-+)?$/', $line, $mts))
			{
				$this->insert(new HRule($this, $line, $mts[1]));
				continue;
			}
	...
	function getAnchor($text, $level)
	{
		...
		// Add heding
		return array($text, $this->count > 1 ? "\n" . $top : '', $id, $autoid);
	}
