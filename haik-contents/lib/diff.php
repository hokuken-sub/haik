...
function diff_style_to_css($str = '')
{
	...
	$str = preg_replace('/^\-(.*)$/m', '<span class="text-danger">$1</span>', $str);
	$str = preg_replace('/^\+(.*)$/m', '<span class="text-info"  >$1</span>', $str);
...
<div class="row">
	<div class="col-sm-6">
		<p>
			<code>L</code> : between backup data and stored page data.<br>
			<code>R</code> : between backup data and your post data.
		</p>
	</div>
	<div class="col-sm-6">
		<p>
			<code>-</code> : deleted line<br>
			<code>+</code> : added line
		</p>
	</div>
</div>
<table class="style_table table table-hover">
 <tr>
  <th>L</th>
  <th>R</th>
  <th>Text</th>
...
EOD;
		$tags = array('th', 'th', 'td');
		foreach ($arr as $_obj) {
			$deleted = ($_obj->get('left') === '-' OR $_obj->get('right') === '-');
			$added   = ($_obj->get('left') === '+' OR $_obj->get('right') === '+');
			$tr_class = $added ? 'success' : ($deleted ? 'danger' : '');
			
			$do_update_diff_table .= '<tr class="'.$tr_class.'">';
