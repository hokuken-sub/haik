...
function links_update($page)
{
	...
	if ($time) {
		...
		if (! empty($rel_new)) {
    			$fp = fopen($rel_file, 'w')
    				or die_message('cannot write ' . h($rel_file));
...
function links_init()
{
	...
	foreach (get_existpages() as $page) {
		...
		if (! empty($rel)) {
			$fp = fopen(CACHE_DIR . encode($page) . '.rel', 'w')
				or die_message('cannot write ' . h(CACHE_DIR . encode($page) . '.rel'));
			fputs($fp, join("\t", $rel));
			fclose($fp);
		}
	}
	...
	foreach ($ref as $page=>$arr) {
		$fp  = fopen(CACHE_DIR . encode($page) . '.ref', 'w')
			or die_message('cannot write ' . h(CACHE_DIR . encode($page) . '.ref'));
...
function links_add($page, $add, $rel_auto)
{
	...
	foreach ($add as $_page) {
		...
		if ($is_page || ! $all_auto) {
			$fp = fopen($ref_file, 'w')
				 or die_message('cannot write ' . h($ref_file));
			fputs($fp, $ref);
			fclose($fp);
		}
	}
...
/* Location: /haik-contents/lib/link.php */