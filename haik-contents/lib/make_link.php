...
class InlineConverter
{
	...
	function replace($arr)
	{
		...
		$this->result[] = ($obj !== NULL && $obj->set($arr, $this->page) !== FALSE) ?
			$obj->toString() : make_line_rules(h($arr[0]));
...
class Link
{
	...
	function setParam($page, $name, $body, $type = '', $alias = '')
	{
		...
		if (! PKWK_DISABLE_INLINE_IMAGE_FROM_URI &&
			...
			$alias = '<img src="' . h($alias) . '" alt="' . $name . '" />';
		} else if ($alias != '') {
			if ($converter === NULL)
...
EOD;
	...
	function toString()
	{
		...
		if ($str !== FALSE) {
			...
			$body = (($body == '') ? '' : '{' . $body . '}') . ';';
			return make_line_rules(h('&' . $this->plain) . $body);
		}
	}
}
...
EOD;
	...
	function set($arr, $page)
	{
		...
		return parent::setParam($page, h($name),
...
EOD;
	...
	function set($arr, $page)
	{
		...
		return parent::setParam($page, h($name), '', 'url', $alias);
	}
...
{
	global $script, $vars, $link_compact, $related, $defaultpage;

	$s_page = h(strip_bracket($page));
	...
	if ($isautolink || is_page($page)) {
		...
		return $al_left . '<a ' . 'href="' . $script . ($defaultpage === $page ? '' : ('?' . $r_page)) . $anchor .
		...
		$retval = '<a href="' .
			$script . '?cmd=edit&amp;page=' . $r_page . '" class="noexists">' . $s_alias .
			'?</a>';
