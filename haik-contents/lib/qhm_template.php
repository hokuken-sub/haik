...
class QHM_Template {
	...
	/** javascript 用データをセットする。セットする変数は ORGM */
	...
	function set_js_value($key, $value = NULL)
	{
		...
		$ORGM = $this->getv('ORGM');
		if ($ORGM === FALSE) $ORGM = array();
		$ORGM = array_merge($ORGM, $data);
		$this->setv('ORGM', $ORGM);
	 ...
	 */
	function set_first_image($url, $force = FALSE)
	{
		if ( ! is_url($url))
		{
			return FALSE;
		...
		if (($force OR ! $this->getv('first_image')) && is_image($url))
		{
			$this->setv('first_image', $url);
			
			//編集者の場合、プレビュー用にfirst_image をセットする
	...
	function read($filename) {
	    ...
	    if (isset($this->values['ORGM']))
	    {
	    	$js = $this->getv('ORGM');
	    	$js = '<script type="text/javascript">
if (typeof ORGM === "undefined") ORGM = {};
ORGM = '. json_encode($js) .';
</script>';
		    $this->prependv('common_script', $js);
	    }
		extract($this->values);
		mb_internal_encoding(TEMPLATE_ENCODE);
	
		//tmpTemplateCache を使う
		if ($cachename === FALSE)
		{
			ob_start();
			include($this->tmpTemplateCache);
			$body = ob_get_contents();
			ob_end_clean();
			
			unlink($this->tmpTemplateCache);
			
			echo $body;
		}
		else if($this->create_cache && $this->enable_cache && $this->page) //特定のプラグインで、無効にできる
		{
			//仮出力
			ob_start();
			include($cachename);
	...
	function convert($filename) {
	    ...
	    if ( ! is_writable(CACHE_DIR))
	    {
	    	if (is_writable(session_save_path()))
	    	{
				...
				$tmp_tc_name = tempnam(session_save_path(), 'qtc-');
				file_put_contents($tmp_tc_name, $phpstr);
				
			    $this->tmpTemplateCache = $tmp_tc_name;
	    	...
	    	{
		    	die_message(sprintf(__('%s に書き込み権限がありません。権限を確認してください。'), CACHE_DIR));
	    	}
	    }
	    else if (! file_exists($tcachename) || filemtime($tcachename) < filemtime($filename)) {
	        $phpstr = $this->convert_php($filename);
			if ($this->encode) $phpstr = mb_convert_encoding($phpstr, TEMPLATE_ENCODE);
	 ...
	 *     - '#{...}' を 'echo ...;' に置換
	 *     - '%{...}' を 'echo h(...);' に置換
	 *     - '#include{...}' を 'echo convert_html(get_source(...));'
	 *     - ついでにXML宣言も置換
	 */
	function convert_php($filename) {
	
		if ( ! file_exists($filename)) $filename = SKIN_DIR . 'origami.skin.php';
		
	    $s = file_get_contents($filename);
	    $s = preg_replace('/#\{(.*?)\}/', '<?php echo $1; ?>', $s);
	    $s = preg_replace('/%\{(.*?)\}/', '<?php echo h($1); ?>', $s);
	    $s = preg_replace('/#include\{(.*?)\}/', '<?php echo convert_html(get_source(\'$1\'))?>', $s);
	    return $s;
	}
