...
function plugin_diff_view($page)
{
	...
	if (file_exists($filename)) {
		...
		$msg = '<pre>' . diff_style_to_css(h(join('', file($filename)))) . '</pre>' . "\n";
	} else if ($is_page) {
		$diffdata = trim(h(join('', get_source($page))));
		$msg = '<pre><span class="text-info">' . $diffdata . '</span></pre>' . "\n";
	} else {
...
<form action="$script" method="post" class="form-inline">
 <div>
  ...
  <input type="password" name="pass"   size="12" class="form-control" style="width:auto" />
  <input type="submit"   name="ok"     value="{$qm->m['fmt_btn_delete']}" class="btn btn-danger" />
 </div>
</form>
EOD;
...
/* Location: /app/haik-contents/plugin/diff.inc.php */