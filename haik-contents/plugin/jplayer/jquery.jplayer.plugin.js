$(function(){
	...
	$("div.jquery_jplayer").each(function(){
		...
		new jPlayerPlaylist(
			...
			playList,
			{
				...
				swfPath: "haik-contents/plugin/jplayer",
				supplied: "mp3",
				wmode:"window",
				volume: 0.5,
	...
	.bind($.jPlayer.event.volumechange, function(e) {
		if (e.jPlayer.options.muted) {
			$(".jp-volume-control > i").removeClass("orgm-icon-volume-high").removeClass("orgm-icon-volume-high").addClass("orgm-icon-volume-mute");
		}
		else if (e.jPlayer.options.volume == 1) {
			$(".jp-volume-control > i").removeClass("orgm-icon-volume-mute").removeClass("orgm-icon-volume-low").addClass("orgm-icon-volume-high");
		}
		else {
			$(".jp-volume-control > i").removeClass("orgm-icon-volume-high").removeClass("orgm-icon-volume-mute").addClass("orgm-icon-volume-low");
		}
	});
	...
	.on("mouseenter", function(){
		$(".jplayer-volume-dropdown-menu", this).show();
	})
	.on("mouseleave", function(){
		$(".jplayer-volume-dropdown-menu", this).hide();
	})
	.on("click", ".jp-volume-control", function(){
		return false;
	});
