...
function plugin_ogp_get_defdata()
{
	...
	if ($defaultpage != $page)
	{
		...
		{
			$this_page_title = $vars['page']. ' - '. $site_title;
		}
		$type = 'article';
	}
	else
	...
	$defdata = array(
		...
		'og:url'         => $url,//
		'og:site_name'   => $site_title,//page_title
...
function plugin_ogp_set_template()
{
	...
	{
		if ($content !== FALSE)
			$plugin_head .= "\t" . '<meta property="'. h($prop). '" content="'. h($content). '">'. "\n";
	}
	
	$qt->appendv('plugin_head', $plugin_head);

    //head prefix を追加
    $head_prefix = "og: http://ogp.me/ns# fb: http://ogp.me/ns/fb# {$type}: http://ogp.me/ns/{$type}#";
    $qt->setv_once('head_prefix', $head_prefix);
...
$(function(){
	...
	if ($ogp_thumb.length > 0) {
		if (typeof ORGM.first_image != "undefined") {
			$ogp_thumb.attr("src", ORGM.first_image);
		}
	}
});
</script>
';
	...
	$qt->appendv_once('plugin_ogp_preview_plugin_script', 'plugin_script', $plugin_script);
...
function plugin_ogp_get_defaultimage()
{
	...
	{
		return dirname($script). '/' . IMAGE_DIR . 'ogp_default.png';
	}
}

/* End of file ogp.inc.php */
/* Location: /app/haik-contents/plugin/ogp.inc.php */