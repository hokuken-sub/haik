...
//		if (FALSE === ($this->conn_id = @ftp_connect($this->hostname, $this->port)))
	 ...
	 * @return	bool
	 */
	function mkdir($path = '', $permissions = NULL, $recursive = FALSE)
	{
		if ($path == '' OR ! $this->_is_conn())
		{
			return FALSE;
		...
		if ($recursive)
		{
			...
			foreach ($pathes as $p)
			{
				...
				if (@ftp_chdir($this->conn_id, $fullpath)) {
					...
					if (@ftp_mkdir($this->conn_id, $p)) {
						ftp_chdir($this->conn_id, $p);
					}
				}
			}
			$this->changedir($pwd);
	...
	function download($locpath, $rempath, $mode = 'auto')
	{
		...
		$mode = ($mode == 'ascii') ? FTP_ASCII : FTP_BINARY;
//var_dump("rem is exists:", $this->is_exists($rempath));
//var_dump("local is exists:", file_exists($locpath));
//var_dump("locpath", $locpath, "rempath", $rempath);
		$result = ftp_get($this->conn_id, $locpath, $rempath, $mode);
//var_dump("download result:", $result);exit;
