...
class LocalAdapter {
	...
	function mkdir($dir, $permission = NULL, $recursive = FALSE)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->mkdir($dir, $permission, $recursive);
		}
	...
	*/
	...
	function chmod($path, $permission)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->chmod($path, $permission);
		}
	...
	*/
	...
	function lsR($path = '.')
	{
		...
		if( $dh = opendir($path))
		{
			while (false !== ($file = readdir($dh)))
			{
			    ...
			    $tmppath = rtrim($path, '/') . '/' . $file;
			    if (is_dir($tmppath))
			    {
			    	$retarr[] = $tmppath;
			        $retarr = array_merge($retarr, $this->lsR($tmppath));
			    }
			    else
			    {
			        $retarr[] = $tmppath;
	...
	function delete_dir($filepath)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->delete_dir($filepath);
		}
		
		$files = $this->lsR($filepath);
		...
		$dirs = array_flip(array_flip($dirs));
		
		//strlen asc
		usort($dirs, array($this, 'cmpStrlen'));
	...
	/** compare function */
	function cmpStrlen($str_a, $str_b)
	{
	...
	function is_exists($path)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->is_exists($path);
		}
	...
	function file_get_contents($file)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->file_get_contents($file);
		}
	...
	function write($fpath, $str, $permission = null)
	{
		...
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->write($fpath, $str, $permission);
		}
	...
	function upload($locpath, $rempath, $mode = 'auto', $permissions = NULL)
	{
		if ( ! is_null($this->ftp))
		{
			return $this->ftp->upload($locpath, $rempath, $mode, $permissions);
		}
...
class Local_FTP extends CI_FTP {
	...
	function lsR($path, $option = '-a ')
	{
		$all_files = array();
		
		$pwd = $this->pwd();
		
		$files = $this->ls($path, $option);
		...
		foreach ($files as $file)
		{
			...
			if ($this->is_dir($file))
			{
				$all_files = array_merge($all_files, $this->lsR($file, $option));
			}
	 ...
	 *   チェックファイルを設置する
	 */
	function putChecker($tmpdir = null) {
		...
		$tmpfile = tempnam($tmpdir, 'qhm_fs-');
		$stream ='test';
		file_put_contents($tmpfile, $stream);

		$result = parent::upload($tmpfile, $this->r_fname, FTP_BINARY, 0666);
		unlink($tmpfile);
		return $result;
	}
	...
	*/
	...
	/**
	* 与えられたディレクトリが、Web上のディレクトリと合致するかチェックする。
	* 
	...
	* @return boolean
	*/
	function request_checker($url)
	{
		$url = dirname($url.'dummmy').'/'.$this->r_fname;
		if (file_get_contents($url))
		{
			return TRUE;
		}
		$this->errmsg = '設置先フォルダが間違っています。';
		return FALSE;
	}
	...
	*/
	function file_get_contents($file)
	{
		$tmpfile = tempnam(CACHE_DIR, "qhm_fs_ftp-");
	...
	function write($fpath, $str, $permission = NULL)
	{
		...
		$this->upload($tmpfile, $fpath, 'auto', $permission);
	...
	*/
	function upload($locpath, $rempath, $mode = 'auto', $permissions = NULL)
	{
		...
		$this->changeidir();
		return parent::upload($locpath, $rempath, $mode, $permission);
	}
...
*/
if ( ! function_exists('local_is_writable'))
{
